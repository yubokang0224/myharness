...
def resolve_shell_command(
    ...
    *,
    ...
    if resolved_platform == "windows":
        # Prefer PowerShell on Windows: Git Bash interprets backslashes in
        # Windows paths as escape sequences, breaking commands like
        # D:\project\... (e.g. \t becomes tab, \S is swallowed).
        powershell = shutil.which("pwsh") or shutil.which("powershell")
        if powershell:
            # PowerShell 5.1 (powershell.exe) does not support && as a pipeline
            # chain operator (that requires PS7/pwsh).  Replace bare && with ;
            # so the command still runs sequentially on both PS versions.
            is_ps5 = "powershell.exe" in powershell.lower() and "pwsh" not in powershell.lower()
            if is_ps5:
                command = _replace_shell_and_operator(command)
...
def _replace_shell_and_operator(command: str) -> str:
    """Replace bare && with ; for PowerShell 5.1 compatibility.

    PS7 (pwsh) supports && natively; PS5.1 (powershell.exe) does not.
    Only replaces && that are outside quoted strings (best-effort).
    """
    # Simple token-level replacement: split on && not inside quotes
    result = _re.sub(r'(?<![&])&&(?![&])', ';', command)
    return result

...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        if self.config.channels.dingtalk.enabled:
            try:
                ...
                for bot in self._resolve_dingtalk_bots(self.config.channels.dingtalk):
                    bot_name = bot.name or "default"
                    default_agent = (bot.default_agent or "").strip()
                    if default_agent and not self._agent_exists(default_agent):
                        logger.error(
                            "DingTalk bot %s skipped: default_agent %s not found",
                            bot_name,
                            default_agent,
                        )
                        continue
                    channel_name = f"dingtalk:{bot_name}"
                    self.channels[channel_name] = DingTalkChannel(
                        bot,
                        self.bus,
                    ...
                    logger.info("DingTalk bot %s enabled as %s", bot_name, channel_name)
            except ImportError as e:
    ...
    @staticmethod
    def _resolve_dingtalk_bots(config: DingTalkConfig) -> list[DingTalkBotConfig]:
        if config.bots:
            return [
                bot.model_copy(update={"name": bot.name or f"bot-{idx + 1}"})
    ...
    def _agent_exists(name: str) -> bool:
        try:
            ...
            logger.exception("Failed to validate DingTalk default_agent %s", name)
            return False

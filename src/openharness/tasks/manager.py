...
from openharness.utils.internal_api_auth import USER_BEARER_TOKEN_ENV
...
class BackgroundTaskManager:
    ...
    def __init__(self) -> None:
        ...
        self._process_envs: dict[str, dict[str, str]] = {}
    ...
    async def create_agent_task(
        ...
        *,
        ...
        record = await self.create_shell_task(
            ...
            task_type=task_type,
            env_updates=(
                {USER_BEARER_TOKEN_ENV: hsjm_user_token.strip()}
                if hsjm_user_token and hsjm_user_token.strip()
                else None
            ),
        )
    ...
    async def _start_process(self, task_id: str) -> asyncio.subprocess.Process:
        ...
        env_updates = self._process_envs.get(task_id)
        process_env = None
        if env_updates:

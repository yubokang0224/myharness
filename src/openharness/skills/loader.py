...
def load_skill_registry(
    ...
    *,
    ...
    for skill in load_user_skills():
        registry.register(skill)
    # Project-level skills: scan both .openharness/skills/ and .agents/skills/
    # (.agents/ is a common convention in projects that ship their own CAD/tool skills)
    if cwd is not None:
        _resolved_cwd = Path(cwd).resolve()
        for _skill_subdir in (".openharness/skills", ".agents/skills"):
            _project_skills_dir = _resolved_cwd / _skill_subdir
            if _project_skills_dir.is_dir():
                for skill in load_skills_from_dirs([_project_skills_dir], source="project"):
                    registry.register(skill)

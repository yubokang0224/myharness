...
class OhmoGatewayBridge:
    ...
    @staticmethod
    def _is_dingtalk_new_dialog_command(message) -> bool:
        return (
            (message.channel == "dingtalk" or str(message.channel).startswith("dingtalk:"))
            and message.content.strip() == "新建对话"
        )

    async def _handle_new_dialog(self, message, session_key: str) -> None:
        await self._interrupt_session(
            session_key,
            reason="starting a new DingTalk conversation",
        )
        reset_session = getattr(self._runtime_pool, "reset_session", None)
